...
AMCData::AMCData(ASFData * asfData)
{
  ...
  *mAsfData = *asfData;
  ...
  {
    segmentDofs = asfData->getSegmentDegreeOfFreedoms(asfSegmentNames.at(i));
    std::cout << asfSegmentNames.at(i) << std::endl;
    std::cout << segmentDofs << std::endl;
    if (asfSegmentNames.at(i) == "root")
    {
      mSegmentNames.push_back("root");
    }
    else if (segmentDofs != Eigen::Vector3d::Zero())
    {
...
bool AMCData::addFrameConfig(Eigen::VectorXd frameConfig)
{
  ...
  else
  {
    ...
    for (std::size_t i=0; i<mSegmentNames.size(); ++i)
    {
      ...
      dofs_flag = mAsfData->getSegmentDegreeOfFreedoms(segmentName);
      if (dofs_flag != Eigen::Vector3d::Zero())
      {
...
int AMCData::readAMCFile(char * fileName)
{
  ...
  // read motion frame
  std::cout << "start to read motion" << std::endl;
  ...
  double frameID;
  std::string segmentName;
  std::pair<std::string, Eigen::VectorXd> segmentConfig;
  ...
  while(std::getline(is, line))
  {
    ...
    if (stream.fail())
    {
      ...
      std::cout << "segmentName = " << segmentName << std::endl;
      ...
      if (segmentName == "root")
      {
        ...
        segmentConfig = std::make_pair(segmentName, dofs);
        frameConfig.push_back(segmentConfig);

        std::cout << "segmentSize = " << segmentConfig.second.size() << std::endl;

      }
      else if (dofs_flag != Eigen::Vector3d::Zero())
      {
        // make sure dofs size is the same as dof flags
        dofs.resize(dofs_flag.size());
        dofs.setZero();

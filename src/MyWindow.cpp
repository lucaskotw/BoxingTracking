                                           ...
                                           int timeStep)
{
  ...
  if (mInputMotion->getSegmentConfig(timeStep, segmentName, &segmentConfig))
  {
    if (segmentName == "root")
    {
      ...
      Eigen::Isometry3d tf;
      tf.linear() = dart::math::eulerZYXToMatrix(
          Eigen::Vector3d(r_t(5), r_t(4), r_t(3))*deg_to_rad);
...
//            << std::endl;
    ...
    else
    {
      ...
      {
        Eigen::Matrix3d refFrame = dart::math::eulerZYXToMatrix(
            Eigen::Vector3d(segmentAxes(2), segmentAxes(1), segmentAxes(0))
            *deg_to_rad);
        //Eigen::Matrix3d refFrame = dart::math::eulerZYXToMatrix(segmentAxes*deg_to_rad);
        // calculate motion transformation
        Eigen::Matrix3d motionRot = dart::math::eulerZYXToMatrix(
            Eigen::Vector3d(segmentConfig(2), segmentConfig(1), segmentConfig(0))
                            *deg_to_rad);
        ...
        //finalTF.linear() = motionRot * refFrame;
...
void MyWindow::timeStepping()
{
  ...
  if (mTimeStepCnt/12 < mInputMotion->getNumFrames() && mTimeStepCnt % 12 == 0)
  {
    ...
    for (int i=0; i<segmentNames.size(); ++i)
    {
      ...
      // set init pose
      transformSegmentAtTimeFrame(segmentName, mTimeStepCnt/12);
    }
  }
